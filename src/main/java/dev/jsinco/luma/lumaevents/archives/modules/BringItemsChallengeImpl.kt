...
@CommandInfo(
    ...
    name = "bic",
    usage = "/<command> bic <player!>"
)
class BringItemsChallengeImpl : CommandModule {
            ...
            if (!storedInteraction.contains(player.uniqueId)) {
                ...
                dialogue.forEachIndexed { index, s ->
                    Bukkit.getAsyncScheduler().runDelayed(EventMain.getInstance(), {
                        player.sendMessage(Util.color(s).colorIfAbsent(TextColor.fromHexString(Util.TEXT_COLOR)))
        ...
        fun tryTakeItemStack(player: Player): Boolean {
            ...
            for (itemStack in itemStacks) {
                if (!player.inventory.containsAtLeast(itemStack, itemStack.amount)) {
                    Util.sendMsg(player,
                        "<reset>You don't have enough</reset> <gold>${LumaItemsUtil.formatEnumerator(itemStack.type.name)}</gold> <reset>for</reset> <#B9DDFF><b>Frosty</b></#B9DDFF>'s <reset>request. <dark_gray>(Missing <gold>${itemStack.amount - player.inventory.all(itemStack.type).values.sumOf { it.amount }}</gold>)</dark_gray>")
                    return false
                }
            }
